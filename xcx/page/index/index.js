...
Page({
  ...
  dakaClick: function () {
    ...
    service.http1(url, {}, function (data) {
      ...
      if (data.err_code == '000') {
        info.sign_status = '1'
        info.day = data.data.day;
        info.point = data.data.point;
        info.nowpoint = data.data.nowpoint;
        info.nowpoint10 = parseInt(data.data.nowpoint / 10);
        info.nowpoint1 = info.nowpoint - info.nowpoint10*10;
        console.log(info);
        that.setData({
          info: info,
          jfFlag: true
  ...
  jfClick: function (e) {
    ...
    picsrc = picsrc.replace(/http/, 'https');
    that.setData({
      jfFlag: false
    });
    if (title == 'riqian') {
